...
namespace StarterAssets
{
    public class GameManager : MonoBehaviour
    {
        ...
        public void Killplayer(DeathControl player)
        {
            lives = 0;
            int left = heartUI.transform.childCount;
            // Remove all heart from the UI
            for (int i = 0; i < left; i++)
            {
                Destroy(heartUI.transform.GetChild(i).gameObject);
            }

            Debug.Log("Player has been killed! Game Over!");
            // Implement gane over logic here (show game over screen, restart game)
            player.Death();
        }
        public void IncreaseLife(DeathControl player)
        {
            lives++;
            Debug.Log("Player has " + lives + " lives remaining.");
            // Add one heart from the UI
            GameObject heart = Instantiate(heartPrefab, heartUI.transform);
            heart.GetComponent<UnityEngine.UI.Image>().sprite = heartImage;
            heart.transform.localPosition = new Vector3((heartUI.transform.childCount - 1) * 50, 0, 0);
